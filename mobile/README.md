...
1. **Use the supported Node.js version**
   ...
   Expo SDK 51/52 runs on Node 18–24, and the project now advertises that
   range in `package.json`/`package-lock.json` to avoid `npm warn EBADENGINE`
   when using Node 24. The repo is pinned to Node 24.11.1 via `.nvmrc` so
   `nvm use` will switch you to a supported runtime. If you are on macOS and
   see `spawn /usr/local/bin/node EAGAIN` from `npx expo start`, switch to the
